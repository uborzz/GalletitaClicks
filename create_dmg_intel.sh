#!/bin/bash
# Script para crear un DMG personalizado con icono y fondo para GalletitaClicks (Intel)

echo "Creando DMG personalizado para Intel con icono y fondo..."

# Verificar que existe la aplicación
if [ ! -d "dist/GalletitaClicks.app" ]; then
    echo "Error: No se encontró dist/GalletitaClicks.app"
    echo "Por favor, ejecuta primero: ./build_mac_intel.sh"
    exit 1
fi
...
3. Para permitir la ejecución (IMPORTANTE - después del paso 2):
   ...
   OPCIÓN A - Si aparece "Abrir igualmente":
   ...
   OPCIÓN B - Si NO aparece "Abrir igualmente" (Macs Intel):
   → Abre Terminal y ejecuta:
     sudo spctl --master-disable
   → Luego intenta abrir la aplicación de nuevo
   → Después, vuelve a activar Gatekeeper:
     sudo spctl --master-enable
   
   OPCIÓN C - Alternativa (más segura):
   → Abre Terminal y ejecuta:
     xattr -cr /Applications/GalletitaClicks.app
   → Luego intenta abrir la aplicación de nuevo
   → Si aún no funciona, ejecuta:
     sudo xattr -rd com.apple.quarantine /Applications/GalletitaClicks.app
...
# Firmar la aplicación de forma simple
...
echo "Comprimiendo DMG para Intel..."
rm -f "GalletitaClicks_Intel.dmg"
hdiutil convert "$DMG_TEMP_FILE" -format UDZO -o "GalletitaClicks_Intel.dmg"
